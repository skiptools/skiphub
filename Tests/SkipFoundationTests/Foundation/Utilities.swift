...
extension NSCalendar { func _bridgeToSwift() -> Calendar { self as Calendar } }
extension NSCharacterSet { func _bridgeToSwift() -> CharacterSet { self as CharacterSet } }
extension NSIndexSet { func _bridgeToSwift() -> IndexSet { self as IndexSet } }
...
public func withTemporaryDirectory<R>(functionName: String = #function, block: (URL, String) throws -> R) throws -> R {
    ...
    let fname = (testBundleName() ?? "TestBundle") + "-" + String(functionName[..<idx]) + "-" + NSUUID().uuidString
        ...
        for (i, x) in instances.enumerated() {
            for (j, y) in instances.enumerated() {
                ...
                if predicted {
                    ...
                    // or unless the hash equality oracle is wrong.
                    XCTAssertNotNil(
                        (0..<10).first { hash(x, salt: $0) != hash(y, salt: $0) },
                        """
                        hash(into:) expected to differ, found to match
                        lhs (at index \(i)): \(x)
                        rhs (at index \(j)): \(y)
